...
её обработать. confirm_users (необязательный список id/@username) --
ДОПОЛНИТЕЛЬНО разрешает нажимать кнопки именно этим людям, независимо от
того, являются ли они формальными админами чата в Telegram. Используй это по
умолчанию, когда просят дать право подтверждать конкретным людям (не только
"админам группы") -- это надёжнее, чем полагаться на target+проверку "админ
ли он": Telegram не всегда отдаёт полный список админов чата вызывающему
аккаунту, и тогда даже настоящий админ при нажатии молча получает отказ без
всякой подсказки почему (реальный случай 2026-08-31: друг-админ группы не
смог подтвердить карточку). Это же поле наследует и verify+action=delete при
эскалации "не уверен" человеку (см. verify ниже) -- тот же target/confirm_users
